  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <div
        ...
        className="scrollbar-hide flex snap-x snap-mandatory overflow-x-auto py-4"
      ...
      >
        {childrenArray.map((child, index) => (
          <div
            key={index}
            className="flex w-full flex-shrink-0 snap-center items-start justify-center px-2"
          >
            <div className="w-full max-w-sm">{child}</div>
          </div>
        ))}
      </div>
      ...
      {showArrows && totalSlides > 1 && (
        <>
          <Button
            variant="outline"
            size="icon"
            className="absolute top-1/2 left-2 z-10 -translate-y-1/2 bg-white/90 shadow-lg hover:bg-white"
            onClick={prevSlide}
          >
          ...
          <Button
            variant="outline"
            size="icon"
            className="absolute top-1/2 right-2 z-10 -translate-y-1/2 bg-white/90 shadow-lg hover:bg-white"
            onClick={nextSlide}
          >
      ...
      {showDots && totalSlides > 1 && (
        <div className="mt-12 mb-6 flex justify-center space-x-2">
